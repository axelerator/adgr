...
*/
//************************************************
PhotonMap :: PhotonMap( const int max_phot ): vertexArray(0), colorArray(0), displayList(0)
//************************************************ 
{
  stored_photons = 0;
  prev_scale = 1;
...
//**********************************************
{
  ...
  if (np.found<8)
  {
      free(np.dist2);
      free(np.index);
//       std::cerr << "Not enough photons found!" << std::endl;
      return;
  }
...
void PhotonMap::generateVertexArray(double scale)
{
    if (vertexArray) {
        delete vertexArray;
    }
    ...
    vertexArray = new GLfloat[stored_photons*3];
    colorArray = new GLfloat[stored_photons*3];
    for (int i=0; i < stored_photons; ++i) {
        colorArray[i*3]   = photons[i].power[0] * scale;
        colorArray[i*3+1] = photons[i].power[1] * scale;
        colorArray[i*3+2] = photons[i].power[2] * scale;
        
        vertexArray[i*3]   = photons[i].pos[0];
        vertexArray[i*3+1] = photons[i].pos[1];
        vertexArray[i*3+2] = -photons[i].pos[2];
    }
}

void PhotonMap::drawGL()
{
  if (vertexArray) {
    if (!displayList) {
      ...
      glDrawArrays(GL_POINTS, 0, stored_photons);
      glEndList();
    }
    else {
